...
**Cron schedule (2026-09-21):** `vercel.json` now has **4** entries — one CDT-offset + one CST-offset pair per target, so each weekday group always fires at its target local time regardless of CDT/CST with no manual twice-yearly update:
- Mon–Thu (12:30 PM target): `30 17 * * 1-4` (CDT) and `30 18 * * 1-4` (CST)
- Friday (10:00 AM target): `0 15 * * 5` (CDT) and `0 16 * * 5` (CST)

Whichever entry corresponds to the wrong local time that day is caught by the target-hour gate below and no-ops; the other sends and sets `reminder_last_sent_date`, which stops its counterpart from ever double-sending that day. Also supports a `?dryRun=1` query param (still requires the `CRON_SECRET` bearer auth) that computes and returns the would-be recipient list without sending anything or touching the gates/dedup flag — safe to run anytime against real data to sanity-check the filtering logic.

**⚠ Vercel plan requirement:** per 16.2, the Vercel Hobby plan was confirmed (2026-09-11, via dashboard) to cap a project at **2** cron jobs total. This 4-entry schedule exceeds that cap — Daniel explicitly chose to keep full DST auto-adjustment over dropping to a simpler 2-entry (no-DST-safety) schedule, so **this requires the project to be on a plan that allows ≥4 cron jobs (e.g. Vercel Pro) before deploying**, or Vercel will likely reject/truncate the `crons` array at deploy time. Not yet confirmed live — check the Vercel dashboard after the next deploy.
...
**Send-outcome tracking (2026-09-11):** `reminder_last_sent_date` is only set if at least one recipient's send actually succeeds (`results.some(r => r.ok)`), not merely attempted. A total Resend outage during the earlier (target-hour) run therefore leaves the flag unset so the later same-target run retries; a partial failure (some recipients succeed) still counts as sent for the day, same as before — those specific failed recipients are not retried until the next day.

**Status as of 2026-09-21:** working, pending plan-cap verification — env vars configured, `notify.beardint.com` verified in Resend, DST-safe per-weekday-target cron schedule (4 entries) just deployed to `vercel.json`; needs confirmation the Vercel plan actually accepts 4 cron jobs (see ⚠ above). Verify with `curl -H "Authorization: Bearer <CRON_SECRET>" "https://<deployed-url>/api/daily-reminder?dryRun=1"` and check the JSON response, or the Resend dashboard for real sends.
...
- **Confirmed on Vercel Hobby plan (2026-09-11)**, per the dashboard plan badge. Hobby caps a project at 2 cron jobs total and doesn't allow sub-daily/exact-minute schedules per job. As of 2026-09-21 `vercel.json` has **4** cron entries (see 5.10) to keep DST auto-adjustment for the new per-weekday reminder targets — this exceeds the confirmed Hobby cap and has **not yet been verified to actually deploy/run** on the current plan. Check the Vercel dashboard cron list after the next deploy; if entries are rejected or missing, either upgrade to a plan allowing ≥4 cron jobs, or fall back to a 2-entry schedule (one per weekday group, no DST auto-adjustment — accept a ±1hr manual nudge at DST changes, next one 2026-11-01).
