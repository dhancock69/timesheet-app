...
**Cron schedule (2026-09-11):** two entries in `vercel.json` — `30 18 * * 1-5` and `30 19 * * 1-5` (18:30 and 19:30 UTC), so the reminder always fires at 1:30 PM America/Chicago regardless of CDT/CST, with no manual twice-yearly schedule update. Whichever entry corresponds to the wrong local time that day is caught by the "before 1:30 PM" gate below and no-ops; the other sends and sets `reminder_last_sent_date`, which stops the first from ever double-sending. Also supports a `?dryRun=1` query param (still requires the `CRON_SECRET` bearer auth) that computes and returns the would-be recipient list without sending anything or touching the gates/dedup flag — safe to run anytime against real data to sanity-check the filtering logic.
...
**Send-outcome tracking (2026-09-11):** `reminder_last_sent_date` is only set if at least one recipient's send actually succeeds (`results.some(r => r.ok)`), not merely attempted. A total Resend outage during the 1:30 PM run therefore leaves the flag unset so the other cron entry (the following hour) retries; a partial failure (some recipients succeed) still counts as sent for the day, same as before — those specific failed recipients are not retried until the next day.

**Status as of 2026-09-11:** fully working — env vars configured, `notify.beardint.com` verified in Resend, DST-safe dual-cron schedule live in production. Verify with `curl -H "Authorization: Bearer <CRON_SECRET>" "https://<deployed-url>/api/daily-reminder?dryRun=1"` and check the JSON response, or the Resend dashboard for real sends.
...
- **Confirmed on Vercel Hobby plan (2026-09-11)**, per the dashboard plan badge. Hobby caps a project at 2 cron jobs total and doesn't allow sub-daily/exact-minute schedules per job — the daily-reminder's two-entry DST workaround (see 5.10) already uses both of those slots, so there's no room left for a third cron job without upgrading to Pro.
