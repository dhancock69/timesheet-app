...
function ManagerView({employees,projects,settings}) {
  ...
  const exportTimesheets=async()=>{
    ...
    setStatus("Building Excel files…");
                  ...
                  {dayEntries.filter(e=>e.reg_hours||e.ot_hours||e.dt_hours).map(e=>{
                    ...
                    return(
                      <div key={e.id} style={{display:"flex",justifyContent:"space-between",fontSize:13,color:C.text,padding:"2px 0"}}>
                        <span><span style={{color:C.gold,fontWeight:700}}>{proj?.project_num}</span><span style={{color:C.muted,fontSize:11,margin:"0 4px"}}>·</span><span style={{color:C.muted,fontSize:11}}>{proj?.task_num}</span>{proj?.expense_type&&<><span style={{color:C.muted,fontSize:11,margin:"0 4px"}}>·</span><span style={{color:C.muted,fontSize:11}}>{proj?.expense_type}</span></>}<span style={{color:C.muted,fontSize:11,margin:"0 4px"}}>·</span><span style={{color:C.text,fontSize:12}}>{proj?.project_name}</span></span>
